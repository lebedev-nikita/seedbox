...
app.post("/api/torrents/upload", async (c) => {
  try {
...
app.get("/api/torrents/:torrentId/files/:fileIndex/download", async (c) => {
  return serveTorrentFile(c, "attachment");
});

app.get("/api/torrents/:torrentId/files/:fileIndex/stream", async (c) => {
